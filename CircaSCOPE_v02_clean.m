%%%%CircaSCOPE v0.2 main script%%%%
%Last update 26.7.2021, developed on MATLAB R2020b by Gal Manella
...
missing_tp = 0; %in case of missing time points (e.g. due to microscopy problem) indicate the indices here. NOTE: the relevant images should be ommitted already in the CellProfiler in order for the tracking to proceed

tic
for i = 1:length(PATH) 
DataStruct(i) = LoadDATA(PATH{i},FirstTP,LastTP,missing_tp);
DataTables(i) = Detrend(DataStruct(i).DataTable_unstacked{:,2:end},48,FirstTP,LastTP,DataStruct(i).DataTable_unstacked{:,1});
end
toc

save([PREFIX,'DataTables.mat'],'DataTables','-v7.3');

%% load existing DataTables structure
load([PREFIX,'DataTables.mat']);

%% export tables to xlsx
tic
%excel doesn't get '\' in sheet names.
well_labels2 = cellfun(@(x) strrep(x,'\mu','u'),well_labels,'UniformOutput',0);

for i = 1:length(PATH)
   xlswrite([OutDir,PREFIX,'raw.xlsx'],DataTables(i).DataTable_raw,well_labels2{i}); 
end
toc
%% PTC construction
%period limits for fit [narrower limits defined afterward for filteration]
per_low1 = 20;
per_high1 = 32;
...
for i = 1:length(PATH)
PTCStruct(i) = PTC_construct(DataTables(i).DataTable_Z,DataTables(i).tp,bef_window,aft_window,cueT,per_low1,per_high1,1);
...
save([PREFIX,'PTCStruct2.mat'],'PTCStruct','-v7.3');

subcol = 2;
subrow = 1;

%% export PTCStruct data to xls

well_labels2 = cellfun(@(x) strrep(x,'\mu','u'),well_labels,'UniformOutput',0); %excel doesn't get '\' in sheet names.

for j = 1:length(PATH)
    T = table(PTCStruct(j).TIPA_null_scaled,PTCStruct(j).fit_peaks_aft_scaled,...
              PTCStruct(j).rsquare_bef,PTCStruct(j).rsquare_aft,...
              PTCStruct(j).Periods_bef,PTCStruct(j).Periods_aft,...
                'VariableNames', {'Old_Phase','New_Phase','r2_before','r2_after','Period_before','Period_after'});
    writetable(T,[OutDir,PREFIX,'.xlsx'],'Sheet',well_labels2{j},'WriteMode','overwritesheet');
end
...
for i = 1:length(PATH)
    conds = find(PTCStruct(i).rsquare_bef > r_th & PTCStruct(i).rsquare_aft > r_th...
        & PTCStruct(i).Periods_aft < per_high & PTCStruct(i).Periods_aft > per_low...
        & PTCStruct(i).Periods_bef < per_high & PTCStruct(i).Periods_bef > per_low);
...
ci = 1;
conds1 = find(PTCStruct(ci).rsquare_bef > r_th & PTCStruct(ci).rsquare_aft > r_th...
    & PTCStruct(ci).Periods_aft > per_low & PTCStruct(ci).Periods_aft < per_high...
    & PTCStruct(ci).Periods_bef > per_low & PTCStruct(ci).Periods_bef < per_high);
...
for i = 1:length(PATH)
    conds = find(PTCStruct(i).rsquare_bef > r_th & PTCStruct(i).rsquare_aft > r_th...
        & PTCStruct(i).Periods_aft > per_low & PTCStruct(i).Periods_aft < per_high...
        & PTCStruct(i).Periods_bef > per_low & PTCStruct(i).Periods_bef < per_high);
    k = length(conds);
...
%% sorted heatmap
%requires DataTables in the workspace
...
for i = 1:length(PATH) 
    ...
    datatable = DataTables(i).DataTable_norm';
...
%requires DataStruct in the workspace
...
for i = 1:length(PATH)
    subplot(subrow,subcol,i);
    histogram(DataStruct(i).Obj_label_life(DataStruct(i).Obj_label_first == FirstTP),100);
    title(well_labels{i});
    xlabel('Age (hours)');
end
suptitle('Survival curve');
...
for i = 1:length(PATH)
   ...
   cells_rhythmic_period(i) = length(find(PTCStruct(i).rsquare_bef > r_th & PTCStruct(i).rsquare_aft > r_th & PTCStruct(i).Periods_aft > per_low & PTCStruct(i).Periods_aft < per_high & PTCStruct(i).Periods_bef > per_low & PTCStruct(i).Periods_bef < per_high));
end
...
myred = [203,24,29]./255; %color for 'after' periods
myblue = [8,69,148]./255; %color for 'before' periods
lb_rot = 25; % rotation of x-label if necessary
...
conds = find(PTCStruct(1).rsquare_bef > r_th & PTCStruct(1).rsquare_aft > r_th...
        & PTCStruct(1).Periods_aft < per_high & PTCStruct(1).Periods_aft > per_low...
        & PTCStruct(1).Periods_bef < per_high & PTCStruct(1).Periods_bef > per_low);
...
periods_bef = NaN(length(periods),length(PATH));
for i= 1:length(PATH)
    conds = find(PTCStruct(i).rsquare_bef > r_th & PTCStruct(i).rsquare_aft > r_th...
        & PTCStruct(i).Periods_aft < per_high & PTCStruct(i).Periods_aft > per_low...
        & PTCStruct(i).Periods_bef < per_high & PTCStruct(i).Periods_bef > per_low);
    ...
    periods_bef(1:length(PTCStruct(i).Periods_bef(conds)),i) = PTCStruct(i).Periods_bef(conds);
end

p_periods = ones(length(PATH),1);
p_periods_aft = ones(length(PATH),1);
p_periods_bef = ones(length(PATH),1);
for i= 2:length(PATH)
[~,p_periods(i)] = ttest2(dlt_periods(:,1),dlt_periods(:,i));
[~,p_periods_aft(i)] = ttest2(periods_aft(:,1),periods_aft(:,i));
[~,p_periods_bef(i)] = ttest2(periods_bef(:,1),periods_bef(:,i));
end
...
function OUT = LoadDATA(PATH,FirstTP,LastTP,OmitTPFlag)
%OmitTPFlag - Omits missing Time Points
...
%%%%%load data:%%%%%
    ...
    if OmitTPFlag
       for i = 1:length(OmitTPFlag)
            TIME_list = {TIME_list{1:OmitTPFlag(i)},['xxx',num2str(OmitTPFlag(i))],TIME_list{OmitTPFlag(i)+1:end}}; %to allow skipping the CueT, without having a lag as a result
       end
    end 
...
function OUT = Detrend(DataTable_raw,MA_window,FirstTP,LastTP,tp)

DataTable_trend = zeros(size(DataTable_raw(FirstTP:LastTP,:)));
    for i = 1:size(DataTable_raw(FirstTP:LastTP,:),2)
        DataTable_trend(:,i) = smooth(DataTable_raw(FirstTP:LastTP,i),MA_window);
    end
    
DataTable_Z = (DataTable_raw(FirstTP:LastTP,:) - DataTable_trend) ./ std(DataTable_raw(FirstTP:LastTP,:),[],1,'omitnan');

OUT.DataTable_raw = DataTable_raw(FirstTP:LastTP,:);
OUT.DataTable_norm = DataTable_raw(FirstTP:LastTP,:) ./ mean(DataTable_raw(FirstTP:LastTP,:),1,'omitnan');
OUT.DataTable_Z = DataTable_Z;
OUT.tp = tp(FirstTP:LastTP,:);
end

function OUT = PTC_construct(DataTable_Z,tp,bef_window,aft_window,cueT,per_low,per_high,RepressFlag)
%%RepressFlag - to repress the appearance of the individual subplotting QC
    ...
    fit_peaks_bef_first = NaN(size(DataTable_Z,2),1);
    rsquare_bef = NaN(size(DataTable_Z,2),1);
    rsquare_aft = NaN(size(DataTable_Z,2),1);
    
    period_range = per_low:0.1:per_high;
    ...
    for ix = 1:size(DataTable_Z,2)
        ...
        [~,temppeaks_bef_first] = findpeaks(fit_bef(tp_high_res >=bef_window(1),ix),10); 
        fit_peaks_bef_first(ix) = temppeaks_bef_first(1);
...
%     %%for changing aft window:
%     TIPA_null_scaled = mod((TIPA_null - (aft_window(1)-120))./Periods_aft,1);
%     fit_peaks_aft_scaled = mod((fit_peaks_aft - (aft_window(1)-120))./Periods_aft,1);
    ...
    OUT.fit_peaks_bef_first = fit_peaks_bef_first;
