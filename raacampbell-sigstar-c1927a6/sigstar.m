function varargout=sigstar(groups,stats,nosort)
    ...
    y=ylim;
    yd=myRange(y)*0.1; %separate sig bars vertically by 5%  %changed to 10% Gal 7.3.19
...
function H=makeSignificanceBar(x,y,p)
    ...
    H(1)=plot(x(:),y,'-k','LineWidth',0.5,'Tag','sigstar_bar'); %was 1.5 Gal 7.3.2019

    %Increase offset between line and text if we will print "n.s."
    %instead of a star. 
    if ~isnan(p)
        offset=0.02; %was 0.005 Gal 7.3.2019
    else
        offset=0.02;
    end

    starY=mean(y)+myRange(ylim)*offset;
    H(2)=text(mean(x(:)),starY,stars, 'FontSize', 12,... %Added: 'FontSize', 12, Gal 7.3.2019
